...
async def process_chat_message(
    ...
    try:
        ...
        # ETAPA 2.5: BUCLE DE RECOLECCIÓN DE PARÁMETROS
            ...
            if not state_data: raise ValueError("Estado de herramienta no encontrado en caché.")

            # Recargamos el contexto desde la BD para asegurarnos de tener todas sus relaciones cargadas
            ...
            # Rellenamos el parámetro que faltaba con la respuesta actual del usuario
            partial_params = state_data.get("partial_parameters", {})
            missing_param_name = state_data.get("missing_parameter_info", {}).get("name")
            if missing_param_name:
                partial_params[missing_param_name.lower()] = question.strip()
            
            # Re-ejecutamos la cadena, ahora con un parámetro más
            db_query_result = await run_db_query_chain(
                ...
                injected_params=partial_params
            )
            
            # --- LÓGICA CRUCIAL AÑADIDA ---
            # Verificamos el resultado de la nueva ejecución
            intent = db_query_result.get("intent")
            final_bot_response = db_query_result.get("final_answer")
            
            if intent == "CLARIFICATION_REQUIRED":
                # Si AÚN faltan parámetros, actualizamos el estado en Redis con la nueva información
                # para que el bucle continúe en el siguiente turno.
                print("CHAT_API: Aún faltan parámetros. Actualizando estado y pidiendo siguiente clarificación.")
                metadata = db_query_result.get("metadata", {})
                state_data["partial_parameters"] = metadata.get("partial_parameters", {})
                state_data["missing_parameter_info"] = metadata.get("missing_parameter_info", {})
                cache_service.set_cache(f"tool_state:{session_id}", state_data, ttl_seconds=300)
            else:
                # ¡Lo tenemos todo! La herramienta se ejecutó (o falló en la ejecución final).
                # Limpiamos el estado para terminar el bucle.
                print("CHAT_API: Todos los parámetros recolectados o herramienta ejecutada. Limpiando estado.")
                cache_service.delete_cache(f"chat_state:{session_id}")
                cache_service.delete_cache(f"tool_state:{session_id}")
            
            # Actualizamos el log con el resultado de este turno
            ...
            if target_context.main_type == ContextMainType.DOCUMENTAL:
                ...
                def run_rag_chain_sync():
                    ...
                    def format_docs_and_log(docs: List[LangchainCoreDocument]):
                        # ... (tu lógica de formateo de docs aquí) ...
                    ...
                    return conversational_rag_chain.invoke({"question": question, "user_provided_name": user_name_from_cache})
