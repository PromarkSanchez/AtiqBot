...
async def login_admin_with_ad_and_handle_mfa(response: Response, form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_crud_db_session)):
    ad_user_attributes = ad_auth_service.authenticate_user_and_get_attributes(form_data.username, form_data.password)
    if not ad_user_attributes: raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Credenciales de Active Directory incorrectas.")
    app_user = await app_user_service.get_or_create_user_after_ad_auth(db, ad_user_attributes, form_data.username)
    if not app_user or not app_user.is_active_local: raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Usuario local no autorizado o inactivo.")
