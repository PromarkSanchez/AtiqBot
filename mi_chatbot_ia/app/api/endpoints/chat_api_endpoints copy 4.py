...
async def process_chat_message_langchain(
    ...
    if llm_override_id:
        print(f"CHAT_EP_INFO: Usando LLM override del cliente API. Config ID: {llm_override_id}")
        llm_model_config = await crud_llm_model_config.get_llm_model_config_by_id(db_crud, llm_override_id)
        if not llm_model_config:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"LLM config ID {llm_override_id} definido en el cliente API no fue encontrado.")
    
    if not llm_model_config:
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="El cliente API debe tener un modelo LLM por defecto configurado para poder chatear.")
    ...
    try:
        ...
        print(f"CHAT_EP_INFO: LLM '{llm_model_config.display_name}' instanciado exitosamente para LangChain.")
    except (NotImplementedError, ValueError) as e:
        print(f"CHAT_EP_ERROR: Fallo al crear el adaptador del LLM: {e}")
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail=str(e))
    # --- LÓGICA DINÁMICA DE LLM (Fin) ---

    log_entry_data: Dict[str, Any] = {
        "user_dni": user_dni_session_id,
        "api_client_name": current_api_client.name,
        "user_message": question,
        "llm_model_used": llm_model_config.display_name,
    ...
    try:
        # AQUÍ EMPIEZA TU LÓGICA DE ORQUESTACIÓN ORIGINAL
            ...
            if attempt_sql_query_flag and context_def_dw_instance_orm and context_def_dw_instance_orm.is_active and \
               context_def_dw_instance_orm.main_type == ContextMainType.DATABASE_QUERY and context_def_dw_instance_orm.db_connection_config_id:
                ...
                for ctx_name in resolved_api_client_allowed_context_names:
                    ...
                    include_in_rag = (current_ctx_orm.main_type == ContextMainType.DOCUMENTAL) or \
                                     (current_ctx_orm.name == DW_CONTEXT_CONFIG_NAME and not attempt_sql_query_flag)
                ...
                if not final_rag_target_context_names:
                    ...
                    if primary_context_for_rag_prompts_orm and primary_context_for_rag_prompts_orm.processing_config:
                        ...
                        rag_prompts = proc_cfg.get("rag_prompts", {})
                        if isinstance(rag_prompts, dict):
                           condense_template_str = rag_prompts.get("condense_question_template", condense_template_str)
                           docs_qa_template_str = rag_prompts.get("docs_qa_template", docs_qa_template_str)
                    ...
                    # ... (Tu lógica de metadata de RAG sin cambios) ...
