...
async def get_cached_llm_adapter(db_crud: AsyncSession, model_config: LLMModelConfig) -> BaseChatModel:
    ...
    if model_id in _cached_llm_instances:
        return _cached_llm_instances[model_id]

    print(f"LLM_CACHE_MISS: Adaptador para model_id={model_id} no en caché. Creando...")
    ...
    _cached_llm_instances[model_id] = adapter_instance
    print(f"LLM_CACHE_HIT: Adaptador para model_id={model_id} creado y cacheado.")
