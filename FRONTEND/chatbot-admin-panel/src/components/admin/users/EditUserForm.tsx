...
// AMPLIAMOS LOS TIPOS IMPORTADOS para incluir el de creación
...
// MODIFICAMOS LAS PROPS para que el formulario sepa en qué modo está
interface EditUserFormProps {
  user: AppUserResponse | null; // Acepta null para el modo creación
  onFormSubmit: (data: AppUserUpdateByAdmin | AppUserLocalCreate) => void;
  onCancel: () => void;
  isSubmitting: boolean;
  isEditMode: boolean; // Prop explícita para saber el modo
}

// AMPLIAMOS EL TIPO DEL FORMULARIO para incluir los campos de contraseña
type FormValues = {
  username_ad: string;
...
const EditUserForm: React.FC<EditUserFormProps> = ({ user, onFormSubmit, onCancel, isSubmitting, isEditMode }) => {
  ...
  useEffect(() => {
    if (isEditMode && user) {
      // Modo Edición: carga los datos del usuario existente
      reset({
      ...
      // Modo Creación: resetea a un formulario vacío
      reset({
        username_ad: '', full_name: '', email: '', is_active_local: true,
        mfa_enabled: false, role_ids: [], password: '', confirm_password: '',
      });
  ...
  const processSubmit: SubmitHandler<FormValues> = (data) => {
    if (isEditMode && user) {
        // PAYLOAD PARA ACTUALIZAR (tu lógica original)
        ...
        const payload: AppUserLocalCreate = {
            ...
            email: data.email,
            is_active_local: data.is_active_local,
  ...
  // Eliminamos el `if(!user) return ...` para permitir renderizar el form vacío

  return (
    <form onSubmit={handleSubmit(processSubmit)}>
      <div className="space-y-6">

        {/* Campo DNI (username_ad), editable solo en modo creación */}
        <div>
          <label htmlFor="username_ad_input" className="block text-sm font-medium text-gray-700 dark:text-gray-300">DNI / Username</label>
          <input
            id="username_ad_input"
            type="text"
            {...register('username_ad', { required: 'El DNI es obligatorio.' })}
            disabled={isEditMode}
            className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm sm:text-sm text-gray-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100 dark:disabled:bg-gray-800 disabled:cursor-not-allowed"
          />
          {errors.username_ad && <p className="mt-1 text-xs text-red-500 dark:text-red-400">{errors.username_ad.message}</p>}
        </div>
        
        {/* Renderizado condicional de los campos de contraseña */}
        {!isEditMode && (
          <>
            <div>
        ...
        <div>
          ...
          {errors.full_name && <p className="mt-1 text-xs text-red-500 dark:text-red-400">{errors.full_name.message}</p>}
        </div>
        {/* Email */}
        <div>
        ...
        <div className="flex items-start">
          ...
          <div className="ml-3 text-sm"> <label htmlFor="is_active_local_checkbox" className="font-medium text-gray-700 dark:text-gray-300"> Activo (Localmente) </label> <p className="text-xs text-gray-500 dark:text-gray-400">Controla si el usuario puede iniciar sesión en el panel de admin.</p> </div>
        </div>
        {/* Checkbox para MFA Habilitado */}
        <div className="flex items-start">
          ...
          <div className="ml-3 text-sm"> <label htmlFor="mfa_enabled_checkbox" className="font-medium text-gray-700 dark:text-gray-300"> Autenticación de Múltiples Factores (MFA) </label> <p className="text-xs text-gray-500 dark:text-gray-400">Requiere un código TOTP para iniciar sesión.</p> </div>
        </div>
        ...
        <div>
          ...
          {isLoadingRoles ? (
            <p className="text-sm text-gray-500 dark:text-gray-400 animate-pulse">Cargando roles disponibles...</p>
          ) : availableRoles.length > 0 ? (
            <div className="space-y-2 p-3 border border-gray-300 dark:border-gray-600 rounded-md max-h-48 overflow-y-auto bg-white dark:bg-slate-700">
              <Controller
                name="role_ids"
                control={control}
                render={({ field }) => (
                    ...
                    {availableRoles.map((role) => (
                      <div key={role.id} className="flex items-center py-1">
                        <input
                          id={`role-${role.id}`}
                          type="checkbox"
                          value={role.id} // El valor del checkbox es el ID del rol
                          checked={field.value?.includes(role.id)} // Comprueba si el ID del rol está en el array 'role_ids' del formulario
                          onChange={(e) => {
                            ...
                            const newRoleIds = e.target.checked
                              ...
                              : currentRoleIds.filter((id) => id !== selectedRoleId);
                            field.onChange(newRoleIds); // Actualiza el campo 'role_ids' en react-hook-form
                          }}
                          className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 dark:border-gray-500 rounded bg-gray-100 dark:bg-gray-700 checked:bg-indigo-500 dark:checked:bg-indigo-500"
                        />
                        <label htmlFor={`role-${role.id}`} className="ml-3 block text-sm text-gray-800 dark:text-gray-200 select-none">
                          {role.name} {/* Asume que el rol tiene una propiedad 'name' */}
                        </label>
                      </div>
                    ))}
                  </>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">No hay roles disponibles para asignar.</p>
          )}
          {errors.role_ids && <p className="mt-1 text-xs text-red-500 dark:text-red-400">{errors.role_ids.message}</p>}
        </div>
      </div>
      
      {/* Botones de Acción (sin cambios respecto a tu código) */}
