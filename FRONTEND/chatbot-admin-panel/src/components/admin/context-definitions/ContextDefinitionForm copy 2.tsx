...
import React, { useEffect,useMemo, useState } from 'react'; // useCallback no se usa, lo quité
...
import { PlusCircleIcon, TrashIcon, InformationCircleIcon, ChevronDownIcon, ChevronUpIcon, DocumentTextIcon, CommandLineIcon  } from '@heroicons/react/24/outline';
...
const ContextDefinitionForm: React.FC<ContextDefinitionFormProps> = ({
  ...
  // --- BLOQUE MODIFICADO/NUEVO: Lógica de carga de tablas de BD ---
  // Reemplazamos la lógica simulada por la real.
const numericDbConnId = watchedDbConnectionId ? parseInt(watchedDbConnectionId, 10) : null;

  const {
      data: dbInspectionData,
      isLoading: isLoadingDbTables, // <-- Este isLoading es real, ya no usamos el de useState
      isError: isErrorDbTables,
  } = useInspectDbConnectionEndpointApiV1AdminDbConnectionsConnIdInspectGet(
      numericDbConnId!,
      {
          query: {
              enabled: !!numericDbConnId && !isNaN(numericDbConnId) && currentMainType === ContextMainType.DATABASE_QUERY,
              staleTime: 1000 * 60 * 30, // 30 mins de cache
              refetchOnWindowFocus: false,
              queryKey: ['dbInspection', numericDbConnId]
          }
      }
  );
  ...
  const dbSchemaTableOptions = useMemo(() => {
      ...
      return dbInspectionData.tables.map(table => ({
                ...
                <div className="space-y-4 pt-2">
                    ...
                    <div>
                     ...
                     <div>
                        ...
                        name="processing_config_database_query.selected_schema_tables_for_llm"
                        control={control}
                        id="selected_schema_tables_for_llm"
                        placeholder={
                            !watchedDbConnectionId ? "Primero selecciona una conexión..."
                            : isLoadingDbTables ? "Inspeccionando base de datos..."
                            : "Selecciona o escribe tablas de la lista"
                        }
                        isDisabled={isLoadingDbTables || !watchedDbConnectionId}
                        options={dbSchemaTableOptions} // Usa nuestra nueva variable reactiva
                      />
