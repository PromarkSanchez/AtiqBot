// Archivo: src/components/webchat/ChatbotPreview.tsx (Versión Final)
...
const ChatbotPreview: React.FC<ChatbotPreviewProps> = ({ config, apiKey, applicationId }) => {
  ...
  const { mutate } = useMutation<any, Error, string>({
    ...
    onError: (error: any) => {
        ...
        if (messages.every(msg => !msg.text.includes(errorText))) { setMessages((prev) => [...prev, { text: `**Error:** ${errorText}`, isUser: false }]); }
    },
  ...
  const handleOpenChat = useCallback(() => {
    ...
    if (apiKey) {
      setIsChatOpen(true);
      if (messages.length === 0) { setIsTyping(true); mutate('__INICIAR_CHAT__'); }
    } else {
      setIsChatOpen(true);
      setMessages([{ text: '**Error:** Introduce un Client ID válido para activar la vista previa.', isUser: false }]);
    }
  }, [isChatOpen, apiKey, messages.length, mutate]);

  useEffect(() => { setIsChatOpen(initialState === 'open'); }, [initialState]);

  // ---> UNICO CÓDIGO NUEVO <---
  // Escucha la orden de abrir desde el script de embed y avisa de su estado
  useEffect(() => {
    const handleMessageFromParent = (event: MessageEvent) => {
      if (event.data?.type === 'ATIQTEC_COMMAND' && event.data.command === 'openChat') {
        handleOpenChat();
      }
    };
  ...
  // Este useEffect para avisar al padre es de tu código original y es correcto
  ...
  // El JSX es exactamente el tuyo original
  return (
      ...
      <div className="relative w-full h-full">
        <div className={`w-full h-full transition-all duration-300 ease-in-out origin-bottom-right ${isChatOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}>
          <div className="w-full h-full bg-white dark:bg-slate-800 rounded-xl shadow-lg flex flex-col font-sans overflow-hidden">
            ...
            <main className="flex-grow p-4 space-y-4 overflow-y-auto">{messages.map((msg, index) => (<div key={index} className={`flex items-start gap-3 ${msg.isUser ? 'justify-end' : ''}`}>{!msg.isUser && <div className="flex-shrink-0"><Avatar size="small" /></div>}<div className={`rounded-lg p-3 max-w-[85%] prose prose-sm dark:prose-invert max-w-none ${msg.isUser ? 'text-white' : 'bg-gray-200 dark:bg-slate-700 text-gray-800 dark:text-gray-200'}`} style={{ backgroundColor: msg.isUser ? theme.primaryColor : undefined }}>
            <ReactMarkdown
              components={{
                a: ({ node, ...props }) => (
                  <a
                    ...
                    // Clases para que se vea como un enlace y para que el texto se corte
                    className={`
            ...
            >
              {msg.text}
                ...
                </div></div>))}{isTyping && (<div className="flex items-start gap-3"><div className="flex-shrink-0">
                  <Avatar size="small" /></div><div className="bg-gray-200 dark:bg-slate-700 rounded-lg">
                    <TypingIndicator /></div></div>)}<div ref={messagesEndRef} />
            </main>
